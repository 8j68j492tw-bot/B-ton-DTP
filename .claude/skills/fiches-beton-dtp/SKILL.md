...
- **Catalogue local `PUMP_CATALOG`** (même principe que `PRODUCT_CATALOG`, fonction `pumpCatalogSuggestionsFor`) : auto-complétion en tapant 2+ caractères dans "Nom de la pompe", remplit tous les champs. Contient à ce jour 2 pompes de départ, demandées explicitement par l'utilisateur ("deux pompes raccordées au site") :
  - **Bunker B-100** — petite pompe à béton volumétrique (Bunker Teksped, Italie). Specs tirées de la page caractéristiques officielle du fabricant (`bunker-teksped.com/fr/pr3-pompebeton/b100/car2.html`, le lien PDF direct du fabricant vers `b100.pdf` est mort/404 sur son propre site).
  - **Bunker B-30** — machine à projeter compacte (enduits, mortiers, chapes). Specs tirées de la fiche technique PDF officielle (`bunker-teksped.com/fr/pdf/pr/b30.pdf`, section "Données techniques").
  - Fabricant "Bunker Teksped" pour les deux — `bunker-teksped.com` répond (200) depuis cet environnement, contrairement à `cdnmedia.mapei.com` qui bloque (comparer avec la section PDF béton ci-dessous) ; PDF locaux non téléchargés pour les pompes (`lienFiche` vide) faute de lien PDF valide trouvé — si l'utilisateur en trouve un, suivre le même processus que pour les PDF béton (télécharger, bundler dans `fiches/`, ajouter à `APP_SHELL`).
- **Ce qui n'a PAS été répliqué pour les pompes** (limitation de portée assumée, à étendre seulement si demandé) : pas de revérification quotidienne locale contre le catalogue, pas d'export/import JSON séparé (Paramètres reste scopé béton), pas de suivi d'usage/tri par fréquence (tri simplement alphabétique par nom), pas de translucidité/logo épinglé sur la liste pompes (page opaque classique comme les vues formulaire/détail/paramètres béton). Si l'utilisateur en demande un, s'inspirer du mécanisme béton équivalent plutôt que d'en inventer un nouveau.
