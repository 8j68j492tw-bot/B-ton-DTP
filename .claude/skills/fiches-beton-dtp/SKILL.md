...
- **Catalogue local `PUMP_CATALOG`** (même principe que `PRODUCT_CATALOG`, fonction `pumpCatalogSuggestionsFor`) : auto-complétion en tapant 2+ caractères dans "Nom de la pompe", remplit tous les champs **y compris `photoUrl`** (photo produit intégrée en base64, comme `DTP_LOGO`). Contient à ce jour 2 pompes de départ, demandées explicitement par l'utilisateur ("deux pompes raccordées au site") :
  ...
  - **Rattrapage pour les pompes déjà pré-remplies avant l'ajout de ces PDF** : comme le pré-remplissage automatique (voir plus haut) ne se redéclenche jamais après la création de la clé `pompes-beton`, un utilisateur ayant déjà ouvert la section Pompes avant cet ajout se serait retrouvé avec des pompes sans `lienFiche` de façon permanente. Le `useEffect` de chargement des pompes fait donc un rattrapage ciblé au chargement : pour toute pompe dont l'`id` suit le format `catalog-seed-<index>` (donc non renommée/recréée manuellement) et dont `lienFiche` est vide, si l'entrée correspondante de `PUMP_CATALOG` a maintenant un `lienFiche`, on le reporte sur la pompe et on repersiste — sans toucher aux autres champs (couleur/photo/nom personnalisés restent intacts). Réutiliser ce même motif si d'autres champs du catalogue doivent un jour être rattrapés sur des pompes déjà seedées.
